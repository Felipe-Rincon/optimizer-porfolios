            ...
            try:
                ...
                if st.button(Variables_front.optimize, key="markowitz_optimize"):
                    with st.spinner(Variables_front.optimizing):  
                        ...
                        st.write(format_percentage(df_metrics))
    ...
    with tab2:
        ...
        if uploaded_file is not None:
            try:
                ...
                st.write(Variables_front.variable_x)
                var_x_name = st.selectbox(Variables_front.metric_variable_x, options=list(Variables_mapping_optimization.names_to_mapping.keys()), key="black_litterman_var_x")

                st.write(Variables_front.variable_y)
                var_y_name = st.selectbox(Variables_front.metric_variable_y, options=list(Variables_mapping_optimization.names_to_mapping.keys()), key="black_litterman_var_y")

                st.write(Variables_front.variable_z)
                var_z_name = st.selectbox(Variables_front.metric_variable_z, options=list(Variables_mapping_optimization.names_to_mapping.keys()), key="black_litterman_var_z")
